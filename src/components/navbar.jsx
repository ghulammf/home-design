...
function Navbar() {
  ...
  const listLink = link.map((item, index) => (
    <a
      key={index}
      href={item.href}
      className="mx-4 p-1 transition duration-0 hover:duration-300 hover:drop-shadow-xl hover:text-emerald-900 hover:scale-110"
    >
      {item.name}
    </a>
  ));
  ...
  return (
    <>
      <nav className="flex justify-between px-[10%] md:justify-around bg-zinc-200 h-16 md:h-20 items-center sticky top-0 z-10">
        <div>
          <h2>
            <svg
              id="logo-70"
        ...
        <div className="hidden md:block font-medium md:text-lg">{listLink}</div>
        <div className="hidden md:block">
          <button className="bg-emerald-900 p-2 px-5 rounded-full text-white font-medium md:text-lg hover:scale-105 transition-all hover:duration-200">
            Contact
          </button>
        </div>
        ...
        >
          {isOpen ? (
            <>
              ...
              <nav className="w-full bg-zinc-200 absolute right-0 top-16 p-4 rounded-es-lg rounded-ee-lg">
                <div className="flex flex-col items-center md:hidden font-medium ">
                  {listLink}
                </div>
